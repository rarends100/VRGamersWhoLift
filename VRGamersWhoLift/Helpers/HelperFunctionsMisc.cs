...
namespace VRGamersWhoLift.Helpers
{
    public class HelperFunctionsMisc
    {
        ...
        public static ProfileViewModel PopulateOtherProfileViewModelData(string OtherUserId, VRGamersWhoLiftContext context)
        {
            ProfileViewModel profile = new ProfileViewModel();

            //id
            profile.UserId = OtherUserId;
            string userName = context.User.Where(u => u.Id == OtherUserId).Select(u => u.UserName).FirstOrDefault()!;
